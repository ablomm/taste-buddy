...
const RecipeReviews = ({userID, username, recipeID, updateRating}) => {
    ...
    const [sortBy, setSortBy] = useState<SortByOptions>(SortByOptions.DateDescending);
    ...
    async function retrieveReviews(page, sortBy, recipeID) {
        const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL || "http://localhost:8080"}/recipe/reviews?page=${page}&orderBy=${sortBy}&recipeID=${recipeID}`, {
            method: 'GET',
            headers: {
    ...
    async function handleDelete() {
        const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL || "http://localhost:8080"}/recipe/delete-review?recipeID=${recipeID}&userID=${userID}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    async function submitReview() {
        if (!userReview) { // Check if the user has already submitted a review
            ...
            const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL || "http://localhost:8080"}/recipe/saveReview`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
    ...
    async function getRecipeRating(recipeID: number) {
        const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL || 'http://localhost:8080'}/recipe/get-recipe-rating?recipeID=${recipeID}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    async function checkCommentExists() {
        const response = await fetch(`${process.env.EXPO_PUBLIC_SERVER_URL || 'http://localhost:8080'}/recipe/review-by-user?userID=${userID}&recipeID=${recipeID}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    return (
        <View>
          {/* Review Input Field */}
            {
                userReview ?
                    <UserReview review={userReview} currentUserID={userID} handleDelete={handleDelete}/>
                    :
                    <View>
            <StarRating
                rating={rating}
          ...
          {/* Sort Reviews Button */}
            {reviews.length > 0 && <TouchableOpacity onPress={() => setSortByModalVisible(true)} style={{padding: 5}}>
                ...
                <TouchableWithoutFeedback onPress={() => setSortByModalVisible(false)}>
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            ...
                            <ModalButton onClick={() => handleSortOptionPress(SortByOptions.DateDescending)}
                                         text="Date Desc"/>
                            <ModalButton onClick={() => handleSortOptionPress(SortByOptions.DateAscending)}
                                         text="Date Asc"/>
                            <ModalButton onClick={() => handleSortOptionPress(SortByOptions.RatingDescending)}
                                         text="Rating Desc"/>
                            <ModalButton onClick={() => handleSortOptionPress(SortByOptions.RatingAscending)}
                                         text="Rating Asc"/>
                        </View>
                    </View>
                ...
                <View>
                    {reviews.map((review, index) => {
                        if (review.userID != userID) {
                            return <UserReview key={index} currentUserID={userID} review={review}
