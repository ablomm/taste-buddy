...
export function FilterBar({tags, onSelectFilter, onSelectOption, onChange}) {
    ...
    function handleSelectedFilter(option: string) {
        if(selectedTags.includes(option)) {
            setSelectedTags(prevSelectedTags => prevSelectedTags.filter(tag => tag !== option));
    ...
    return (
      <View style={styles.container}>
          <TouchableOpacity style={styles.sortButton} onPress={() => setSortByModelVisible(true)}>
              <Text style={{color: 'blue', fontWeight: 'bold'}}>Sort By: {selectedOption}</Text>
          </TouchableOpacity>
          <Modal
              animationType="slide"
              transparent={true}
          ...
          >
              ...
              <View style={styles.centeredView}>
                  <View style={styles.modalView}>
                      ...
                      <ModalButton onClick={() => handleSortOptionPress('Relevancy')} text="Relevancy" />
                      <ModalButton onClick={() => handleSortOptionPress('Calories')} text="Calories (Low to High)" />
                      <ModalButton onClick={() => handleSortOptionPress('Cook Time')} text="Cook Time (Low to High)" />
                      <ModalButton onClick={() => handleSortOptionPress('Servings')} text="Servings (Low to High)" />
                  </View>
              </View>
              </ TouchableWithoutFeedback>
          </Modal>
              ...
              <TouchableWithoutFeedback onPress={() => setFilterModalVisible(false)}>
                  <View style={styles.centeredView}>
                      <View style={styles.modalView}>
                          ...
                          <FlatList
                              style={styles.list && { maxHeight: Math.ceil(tags.length / 3) * 60 }}
                              data={tags}
                              renderItem={({item}) => (
                                  <FilterButton
                              ...
                              numColumns={3}
                          />
                          <ModalButton text="Done" onClick={() => setFilterModalVisible(false)}/>
                      </View>
                  </View>
              </ TouchableWithoutFeedback>
          </Modal>
